...
class Network():
    ...
    def train(self, x_train, y_train, x_val, y_val, params_dict, early_stopping=True ,return_log=False, out_log=False,save=False):
        ...
        for num_ep in range(self.num_epochs):
            ...
            # The training cannot stop before the first 100 epochs. After that if in the last 10 epochs an improvement of the validation loss
            # of 1% does not occur w.r.t. the 50 epochs before the training is stopped.  
            if early_stopping and num_ep > 200:
                if (np.mean(val_loss_log[-200:]) - np.mean(val_loss_log[-400:-200]))/np.mean(val_loss_log[-600:-300])*100> -5:
    ...
    def grid_search(self, params_dict,x_data, label):
        ...
        for combination in trange(len(combinations)):
            print("\nPerforming training", combination, "out of", combinations.shape[0])
        ...
        with open('train_results.data', 'wb') as filehandle:
            pickle.dump(list(zip(combinations,list_avg_val_losses,list_std_val_losses)), filehandle)
...
if __name__ == "__main__":
    ...
    best_net.load_model(WBh1,WBh2,WBo)
    y_test_est = np.array([best_net.forward(x) for x in x_test])
    mse = (y_test_est - y_test)**2/2
    test_loss = np.mean(mse)
    std_test_loss = np.std(mse)
    print("\n TEST LOSS : ", round(test_loss,2), "±", round(std_test_loss,2), "\n" )
    np.savetxt('mse.txt', np.transpose([x_test,mse]),header='point x in test set,   mse related to x')
    print("The output, i.e. the MSE of the points, is in the file mse.txt") 
    print("\n Please check source/EX1_Matteo_Guida.ipynb for the full work.")
